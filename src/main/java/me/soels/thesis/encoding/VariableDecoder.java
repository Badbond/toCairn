...
public class VariableDecoder {
     ...
     *
     * @param analysisInput the additional analysis input information required to construct the clustering
     ...
     */
    public Clustering decode(AnalysisInput analysisInput, int[] variables, EncodingType encodingType) {
        switch (encodingType) {
            case GRAPH_ADJECENCY:
                return decodeGraphAdjacency(analysisInput, variables);
            case CLUSTER_LABEL:
                return decodeClusterLabel(analysisInput, variables);
            default:
     ...
     */
    public Clustering decodeClusterLabel(AnalysisInput analysisInput, int[] variables) {
        ...
        for (var i = 0; i < variables.length; i++) {
            ...
            clusteringBuilder.addToCluster(analysisInput.getOtherClasses().get(i), clusterNormalizationMapping.get(clusterNumber));
        }
     ...
     * @return the clustering that the given variables represent
     */
    public Clustering decodeGraphAdjacency(AnalysisInput analysisInput, int[] variables) {
        var clusteringBuilder = new ClusteringBuilder();
        ...
        for (var i = 0; i < variables.length; i++) {
            ...
            if (existingCurrent.isPresent() && existingLinked.isEmpty()) {
                ...
                clusteringBuilder.addToCluster(analysisInput.getOtherClasses().get(linkedNode), existingCurrent.get());
                ...
                clusteringBuilder.addToCluster(analysisInput.getOtherClasses().get(i), existingLinked.get());
                ...
                clusteringBuilder.addToCluster(analysisInput.getOtherClasses().get(i), clusterCount);
                nodeClusterPair.put(linkedNode, clusterCount);
                clusteringBuilder.addToCluster(analysisInput.getOtherClasses().get(linkedNode), clusterCount);
                clusterCount++;
